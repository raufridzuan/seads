...
func main() {
	...
	allAdResults, err := internal.RunAdSearch(config)
	if err != nil {
		if len(allAdResults) == 0 {
			log.Fatalf("error running ad search: %v\n", err)
		} else {
			log.Printf("error running ad search but has Adsresults: %d\n", len(allAdResults))
		}
	}
	log.Printf("Process URLScan and Notification and output\n")
	// Submit domains to URLScan
	if internal.EnableURLScan {
		fmt.Println()
		config.URLScanSubmission(allAdResults)
	}

	// Submit notification
	if internal.EnableNotifications {
		fmt.Println()
		config.SendNotifications(allAdResults)
	}
